...
module.exports = function ($state, $rootScope, $timeout, $log, $window, AsideFactory, Auth, ProjectFactory, ActiveProjectService, NavBarService, uiError, $filter) {
    ...
    this.init = function () {
        ...
        that.loading = true;
        that.preferences = {};
        // This ensures that the Hero banner project name is reset
        NavBarService.updateHeading();

        // Apply certain rules depending on users profile i.e. standard | guest
        // Note: this logic cant be applied at the client/index.js level as AngularJS will have already loaded the templates before it
        // can determine is a standard | guest
        Auth.getCurrentUser().$promise
            .then(function (user) {
                that.user = user;
                that.canCreate = (user && user.acl_roles && user.acl_roles.indexOf('standard') !== -1);
                if (user && user.acl_roles && user.acl_roles.indexOf('guest') === -1) {
        ...
        that.predicate = ['+isNew', '-stats.created_at', '-orderByIndex'];
        that.newProject = {
            archived: false,
            isNew: true,
            name: '',
    ...
    this.showNewProjectForm = function () {
        ...
        that.newProject.orderByIndex = that.activeProjects.length + 1;
    ...
    this.openProject = function (projectId, projectName, isOwner) {
        ...
        ActiveProjectService.name = projectName;
