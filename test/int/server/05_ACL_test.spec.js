...
var nonAuthUser = new ProjectAPI();
...
describe('ACL REST userOne Test', function () {
    ...
    before('Initialize User One', function (done) {
        userOne.initialize(USER_ONE_EMAIL, 'Minisap!1', true).then(done).catch(done);;
    });

    before('Initialize User Two', function (done) {
        userTwo.initialize(USER_TWO_EMAIL, 'Minisap!1').then(done).catch(done);;
    });

    before('Initialize nonAuth user', function (done) {
        nonAuthUser.nonAuthInit(true)
            .then(function () {
                done();
            })
            .catch(done);
    ...
    it('Negative - Call DELETE /api/projects/:projectId/asset - should not be able to delete an asset that does not exist', function (done) {
        userOne.deleteAsset(404, projectId, '109cf2ed5356c6bf0a8cf0ff', function (err) {
            if (err) {
                return done(err);
            }
            done();
    ...
    it('Negative - Non authorised users should not be allowed access invites - PUT', function (done) {
        nonAuthUser.acceptInvite(401, projectId, function (err) {
            if (err) {
                done(err);
            }
            done();
    ...
    it('Negative - Non authorised users should not be allowed access invites - DELETE', function (done) {
        nonAuthUser.rejectInvite(401, projectId, function (err) {
            if (err) {
                done(err);
            }
            done();
    ...
    it('Negative - Non authorised users should not be allowed access invites - PATCH', function (done) {
        nonAuthUser.acceptInvitePatch(401, projectId, function (err) {
            if (err) {
                done(err);
            }
            done();
