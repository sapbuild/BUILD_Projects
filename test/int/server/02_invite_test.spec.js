...
var USER_ONE_EMAIL = 'user.1@somedomain.com';
var USER_TWO_EMAIL = 'user.2@somedomain.com';
var USER_THREE_EMAIL = 'user.3@somedomain.com';
...
describe('Invite REST API Test', function () {
    ...
    before('Initialize User Three', function (done) {
        userThree.initialize(USER_THREE_EMAIL, 'Minisap!1').then(done);
    });
    ...
    it('Call POST /api/projects/:projectId/revoke/invite - User one should be allowed to revoke invite', function (done) {
        userOne.revokeInvite(204, projectId, USER_TWO_EMAIL, function (err) {
            if (err) {
                done(err);
            } else {
                done();
    ...
    it('Call POST /api/projects/:projectId/revoke/invite - User one should not be able to delete an email that does not exist', function (done) {
        userOne.revokeInvite(404, projectId, 'invalid@somedomain.com', function (err) {
            if (err) {
                done(err);
            } else {
                done();
    ...
    it('Call POST /api/projects/:projectId/invite - Create same invite again', function (done) {
        userOne.createInvite(201, projectId, {'email_list': [{'email': USER_TWO_EMAIL}]}, function (err, res) {
            if (err) {
                done(err);
            } else {
    ...
    it('Call POST /api/projects/:projectId/revoke/invite - User two should not be allowed to revoke the invite as they are not the owner', function (done) {
        userTwo.revokeInvite(403, projectId, USER_ONE_EMAIL, function (err) {
            if (err) {
                done(err);
            } else {
                done();
