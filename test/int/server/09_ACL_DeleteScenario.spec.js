...
var EMAIL_ADDRESS = 'projects.module@sap.com';
...
describe('Document REST Test cases', function () {
    ...
    before('Initialize userOne', function (done) {
        userOne.initialize(EMAIL_ADDRESS, 'Minisap!1', true)
            .then(function () {
    ...
    before('Initialize User Two userOne', function (done) {
        userTwo.initialize(USER_TWO_EMAIL, 'Minisap!1')
            .then(function () {
    ...
    it('User One change owner', function (done) {
        userOne.changeOwner(204, projectId, {userId: userTwoId}, function (err) {
            if (err) {
                done(err);
            } else {
                done();
    ...
    it('User One should no longer be able to call revoke invite as they are no longer the owner of the project', function (done) {
        userOne.revokeInvite(403, projectId, 'test@test.com', function (err) {
            if (err) {
                done(err);
            } else {
                done();
