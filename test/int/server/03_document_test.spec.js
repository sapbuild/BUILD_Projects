...
describe('Document REST Test cases', function () {
    ...
    before('Initialize userOne', function (done) {
        userOne.initialize(EMAIL_ADDRESS, 'Minisap!1', true).then(done);
    });
    ...
    it('Call GET /userOne/projects/:projectId/document/:assetId/render - Try to render an image that doesnt exist', function (done) {
        userOne.renderAsset(404, projectId, 'ccb37853e7e4588f0a8d01fa', null, function (err, res) {
            if (err) {
                done(err);
            } else {
                expect(userOne.isContentTypeJSON(res)).to.be.true;
